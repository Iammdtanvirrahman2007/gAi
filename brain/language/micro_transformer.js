 ...
 * Tiny browser-native Transformer for learning experiments.
 * Educational micro-model, not a production LLM.
 */
const KEY='gAiMicroTransformerV1';
const CFG={dim:24,heads:4,maxSeq:48,lr:.035};
const clean=s=>String(s||'').replace(/\s+/g,' ').trim();
const tokenize=s=>clean(s).toLowerCase().replace(/[^\p{L}\p{N}'!?.,-]+/gu,' ').split(/\s+/).filter(Boolean);
const START='<s>',END='</s>';
function empty(){return{vocab:[START,END],ids:{[START]:0,[END]:1},E:[],P:[],Wq:[],Wk:[],Wv:[],Wo:[],W1:[],W2:[],b2:[]}}
...
function ensureModel(){let m=load();if(m&&m.vocab&&m.ids&&Array.isArray(m.E)&&Array.isArray(m.P)&&m.Wq)return m;m=empty();const d=CFG.dim;m.E=matrix(m.vocab.length,d);m.P=matrix(CFG.maxSeq,d);m.Wq=matrix(d,d);m.Wk=matrix(d,d);m.Wv=matrix(d,d);m.Wo=matrix(d,d);m.W1=matrix(d*2,d);m.W2=matrix(d,d*2);m.b2=rand(d);save(m);return m}
...
function forward(m,tokens){const ids=tokens.map(t=>m.ids[t]??m.ids[END]).slice(0,CFG.maxSeq),hidden=attentionBlock(m,ids);return{ids,hidden,logits:hidden.map(v=>matvec(m.E,v))}}
function trainStep(m,text){const toks=[START,...tokenize(text),END];addVocab(m,toks);const seq=toks.slice(0,CFG.maxSeq);let loss=0,steps=0;for(let i=1;i<seq.length;i++){const input=seq.slice(Math.max(0,i-12),i),target=m.ids[seq[i]],f=forward(m,input),p=softmax(f.logits[f.logits.length-1]);loss-=Math.log(Math.max(1e-8,p[target]||1e-8));steps++;const ctx=f.hidden[f.hidden.length-1],e=m.E[target];for(let j=0;j<CFG.dim;j++)e[j]+=CFG.lr*Math.max(-.2,Math.min(.2,ctx[j]));const pred=p.indexOf(Math.max(...p));if(pred!==target){const pe=m.E[pred];for(let j=0;j<CFG.dim;j++){const delta=CFG.lr*Math.max(-.1,Math.min(.1,ctx[j]));pe[j]-=delta;e[j]+=delta}}}return{loss:steps?loss/steps:0,steps}}
